...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $taskTable ($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colTask TEXT, $colDate TEXT, $colDateEnd TEXT, $colStatus TEXT)');
  }

  //Fetch Operation: Get all Task objects from database
  Future<List<Map<String, dynamic>>> getTaskMapList() async {
    Database db = await this.database;
    //var result = db.rawQuery('SELECT * FROM $taskTable order by $colDate, $colDateEnd ASC');
    var result =
        db.query(taskTable, orderBy: '$colStatus, $colDate, $colDateEnd');
    return result;
  }
  ...
  Future<List<Map<String, dynamic>>> getInCompleteTaskMapList() async {
    ...
    var result = db.rawQuery(
        'SELECT * FROM $taskTable where $colStatus = "" order by $colDate, $colDateEnd ASC');
    //var result = db.query(taskTable, orderBy: '$colStatus, $colDate, $colDateEnd');
    return result;
  }
  ...
  Future<List<Map<String, dynamic>>> getCompleteTaskMapList() async {
    Database db = await this.database;
    var result = db.rawQuery(
        'SELECT * FROM $taskTable where $colStatus = "Task Completed" order by $colDate, $colDateEnd ASC');
    //var result = db.query(taskTable, orderBy: '$colStatus, $colDate, $colDateEnd');
    return result;
  }
